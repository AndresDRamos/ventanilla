...
const TicketModalResponse = ({ ticket, usuarioQueAtendio }) => {
  ...
  const hasResponse = ticket.atenciones && ticket.atenciones.length > 0;
  const response = hasResponse ? ticket.atenciones[0] : null;
  ...
  return (
    <ResponseViewSection>
      {hasResponse && response.respuesta && (
        <>
          <ResponseViewTitle>Respuesta:</ResponseViewTitle>
          <ResponseText>{response.respuesta}</ResponseText>

          {/* Información de atención como leyenda inmediatamente después de la respuesta */}
          {(hasResponse || ticket.idEstado === 5 || ticket.idEstado === 3) && (
            <AttentionInfo>
              <AttentionInfoRow>
                <AttentionInfoLabel>
      ...
      {/* Solo mostrar información de atención si NO hay respuesta pero sí está atendido/cancelado */}
      {!hasResponse && (ticket.idEstado === 5 || ticket.idEstado === 3) && (
        <AttentionInfo>
          <AttentionInfoRow>
            <AttentionInfoLabel>
      ...
      {/* Información adicional para tickets atendidos */}
      {ticket.idEstado === 4 && (
        <ResponseInfo>
          {response?.calificacion && response.calificacion > 0 && (
            <div>
